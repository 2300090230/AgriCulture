  ...
  try {
    ...
    if (row) {
      gsap.to(row, {
        ...
        duration: 0.3,
        onComplete: async () => {
          const response = await axios.post(`${config.url}/admin/approvefarmer/${sid}`);
          alert(response.data);
          fetchSellers();
        }
      });
    ...
    try {
      ...
      if (row) {
        gsap.to(row, {
          ...
          duration: 0.3,
          onComplete: async () => {
            const response = await axios.put(`${config.url}/farmer/reject/${sid}`);
            alert(response.data);
            fetchSellers();
          }
        });
    ...
    try {
      ...
      if (row) {
        gsap.to(row, {
          ...
          duration: 0.3,
          onComplete: async () => {
            const response = await axios.delete(`${config.url}/farmer/delete?id=${id}`);
            alert(response.data);
            fetchSellers();
          }
        });
                        ...
                        {seller.status === "Pending" && (
                          <>
                            <button 
                              ...
                              className="relative overflow-hidden px-3 py-1.5 bg-green-600 text-white rounded transition-all duration-300 ease-in-out hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 flex items-center"
                            >
